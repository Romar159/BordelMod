...
public class bordelEnrichissementMultiBlock extends TileEntity {
        ...
        if (!worldObj.isRemote) {
            if (hasMaster()) { 
                if (isMaster()) {
                	...
                	//System.out.println("Multiblock ON !\n");
                }
            } else {
    ...
    // On check ici si la structure est correctement formée.
			...
			for (int y = yCoord; y < yCoord + 3; y++)
				for (int z = zCoord - 1; z < zCoord + 2; z++) {
					...
					if (tile != null && (tile instanceof bordelEnrichissementMultiBlock)) {
						if (this.isMaster()) {
							if (((bordelEnrichissementMultiBlock)tile).hasMaster())
								i++;
						} else if (!((bordelEnrichissementMultiBlock)tile).hasMaster())
							i++;
					}
			}
        ...
        for (int x = xCoord - 1; x < xCoord + 2; x++)
            for (int y = yCoord; y < yCoord + 3; y++)
                for (int z = zCoord - 1; z < zCoord + 2; z++) {
                    ...
                    if (tile != null && (tile instanceof bordelEnrichissementMultiBlock)) {
                        ((bordelEnrichissementMultiBlock) tile).setMasterCoords(xCoord, yCoord, zCoord);
                        ((bordelEnrichissementMultiBlock) tile).setHasMaster(true);
                        ((bordelEnrichissementMultiBlock) tile).setIsMaster(master);
                    }
                }
    }
    ...
    public boolean checkForMaster() {
        ...
        return (tile != null && (tile instanceof bordelEnrichissementMultiBlock));
    }
        ...
        for (int x = xCoord - 1; x < xCoord + 2; x++)
            for (int y = yCoord; y < yCoord + 3; y++)
                for (int z = zCoord - 1; z < zCoord + 2; z++) {
                    TileEntity tile = worldObj.getTileEntity(x, y, z);
                    if (tile != null && (tile instanceof bordelEnrichissementMultiBlock))
                        ((bordelEnrichissementMultiBlock) tile).reset();
                }
    }
